...
func (s *DockerSwarmSuite) TestAPISwarmPromoteDemote(c *check.C) {
	...
	c.Assert(err, checker.IsNil)
	c.Assert(res.StatusCode, checker.Equals, http.StatusInternalServerError, check.Commentf("output: %q", string(b)))
...
func (s *DockerSwarmSuite) TestAPIDuplicateNetworks(c *check.C) {
	...
	r1, err := cli.NetworkInspect(context.Background(), n1.ID, false)
	c.Assert(err, checker.IsNil)
	c.Assert(r1.Scope, checker.Equals, "local")

	r2, err := cli.NetworkInspect(context.Background(), n2.ID, false)
	c.Assert(err, checker.IsNil)
