...
func (d *Swarm) SwarmInfo() (swarm.Info, error) {
	...
	res, dt, err := request.DoOnHost(d.Sock(), "/info", request.Method("GET"))
	if res.StatusCode != http.StatusOK {
		return info.Swarm, fmt.Errorf("get swarm info: invalid statuscode %v", res.StatusCode)
	}
	if err != nil {
		return info.Swarm, fmt.Errorf("get swarm info: %v", err)
	}

	if err := json.NewDecoder(dt).Decode(&info); err != nil {
...
func (d *Swarm) CreateService(c *check.C, f ...ServiceConstructor) string {
	...
	scr, err := cli.ServiceCreate(context.Background(), service.Spec, types.ServiceCreateOptions{})
	c.Assert(err, checker.IsNil)
